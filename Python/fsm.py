    ...
    if 1 in accepting:
        ...
        if current in accepting:
            ...
            for i in range(len(edge_values)):
                nfsmaccepts(edge_values[i][0], edges, accepting, visited)
...
# fill in your code here
    if string == "":
        ...
        if key in edges:
            next_states = len(edges[key])
            if next_states == 1:
                current = edges[key][0]
                return nfsmsim(rest, current, edges, accepting)
            else:
                letter_next = rest[0]
                target = 0
                for i in range(next_states):
                    next_key = (edges[key][i], letter_next)
                    if next_key in edges:
                        target = i
                current = edges[key][target]
                return nfsmsim(rest, current, edges, accepting)
        else:
            return False
